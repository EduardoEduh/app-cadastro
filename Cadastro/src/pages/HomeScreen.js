import React, { useState, useEffect } from 'react';
import styled from 'styled-components/native';
//Importando às configurações do banco de dados e o controler
import SQLiteManager from '../DataBase/SQLiteManager';
import CadastrosController from '../DataBase/controller/CadastrosController';

import { TouchableOpacity } from 'react-native';
//Importando a biblioteca picker para pegar uma image do celular
import { launchImageLibrary } from 'react-native-image-picker';

const Body = styled.SafeAreaView`
  flex: 1;
  justify-content: center;
  align-items: center;
  background-color:  rgb(125, 32, 199);
`;

const Scroll = styled.ScrollView`
  flex: 1;
  width: 100%;
`;

const ImageUser = styled.Image`
  width: 100px; 
  height: 100px;
`;

const BoxInputs = styled.View`
  flex: 1;
  align-items: center;
  width: 100%;
  margin-top: 25px;
  background-color: rgb(125, 32, 199);
  border-radius: 10px;
`;

const TextoTitulos = styled.Text`
  font-weight: bold;
  font-size: 17px;
  color: #fff;
  align-self: flex-start;
  margin-left: 22px;
  margin-bottom: 10px;
`;

const Input = styled.TextInput`
  width: 90%;
  height: 40px;
  background-color: #fff;
  border-radius: 12px;
  margin-bottom: 20px;
  font-size: 17px;
`;

const Botao = styled.TouchableOpacity`
  align-items: center;
  justify-content: center;
  width: 45%;
  height: 50px;
  background-color: #FFF;
  border-radius: 12px;
  margin-bottom: 20px;
`;

const TextButton = styled.Text`
  font-weight: bold;
  font-size: 22px;
  color: rgb(125, 32, 199);
`;

function HomeScreen(props) {
  const [ filePath, setFilePath] = useState({});
  const [ image, setImage ] = useState('');
  const [ name, setName ] = useState('');
  const [ sobrenome, setSobrenome ] = useState('');
  const [ dataNascimento, setDataNascimento ] = useState('');
  const [ email, setEmail ] = useState('');
  const cadastros = new CadastrosController();

  useEffect(() => {
    /*Drop para apagar os cadatros
    SQLiteManager.dropDatabase();*/
    
    SQLiteManager.initDB();
  });
 
  
  //Função pacote react-native-image-picker
  const uploadGalery = (type) => {
    let options = {
      mediaType: type,
      maxWidth: 300,
      maxHeight: 550,
      quality: 1,
      includeBase64: true,
    };
    launchImageLibrary(options, (response) => {
      response = response
      console.log('Response = ', response);
      //Verificação de escolha de imagem da galeria
      if (response.didCancel) {
        alert('ATENÇÃO! Você fechou a galeria sem escolher nenhuma foto');
        return;
      //Verificação da permissão para acesso a galeria
      } else if (response.errorCode == 'permission') {
        alert('ATENÇÃO! É Necessário aceitar as permissões de galeria');
        return;
      } else if (response.errorCode == 'others') {
        console.log(response.errorMessage);
        return;
      }
      //Resposta da função da  biblioteca picket imagem
      console.log('base64 -> ', response.assets[0].base64);
      console.log('uri -> ', response.assets[0].uri);
      console.log('width -> ', response.assets[0].width);
      console.log('height -> ', response.assets[0].height);
      console.log('fileSize -> ', response.assets[0].fileSize);
      console.log('type -> ', response.assets[0].type);
      console.log('fileName -> ', response.assets[0].fileName);

      //Formatação para o componente imagem ler "base64"
      setImage('data:'+response.assets[0].type+';base64,'+response.assets[0].base64);
      setFilePath(response.assets[0]);
    });
  };

  //Função de Add dados
  const addList = () => {
    //Verficação se todos os inputs estão preechidos
    if(image && name && sobrenome && dataNascimento && email) {
      const data = {
        image,
        name,
        sobrenome,
        dataNascimento,
        email
      };
      const arrData = [];
      arrData.push(data);
      cadastros.addCadastros(arrData)
        .then(() => {
            alert('Cadastrado com Sucesso!');
            setFilePath({})
            setImage('')
            setName('')
            setSobrenome('')
            setDataNascimento('')
            setEmail('')
            //Puxando os cadastros do SQLite e passando eles para outra tela
            cadastros
            .showCadastros()
            .then((results) => {
              console.log(results)
              /*props.navigation.navigate('LIST', {
                list: results
              });*/
            })
            .catch(() => {
                alert('Error ao Mostrar');
            });
        })
        .catch(() => {
            alert('Error ao Cadastrar');
        });
    } else {
      alert('Preencha Todos os Campos')
    }
  }
  
  //Função do Button Ver Cadastros
  //Puxando do banco de dados para passar todos os cadastros
  const listScreen = ()=>{
    cadastros
    .showCadastros()
    .then((results) => {
      console.log(results)
      props.navigation.navigate('LIST', {
        list: results
      });
    })
    .catch(() => {
        alert('Error ao Mostrar');
    });
  }

  return(
    <Body>
      <Scroll>
        <BoxInputs>
        
            <TextoTitulos>Insira seu avatar(foto)</TextoTitulos>
            
            <TouchableOpacity
              activeOpacity={0.5}
              onPress={() => uploadGalery('photo')}>
              <ImageUser
                source={{uri: filePath.base64 ? 'data:'+filePath.type+';base64,'+filePath.base64 : 'data:image/png;base64,' }}
              />
            </TouchableOpacity>

            <TextoTitulos>Nome:</TextoTitulos>
            <Input onChangeText={e=>setName(e)} value={name}/>
            <TextoTitulos>Sobrenome:</TextoTitulos>
            <Input onChangeText={e=>setSobrenome(e)} value={sobrenome}/>
            <TextoTitulos>Data de Nascimento:</TextoTitulos>
            <Input onChangeText={e=>setDataNascimento(e)} value={dataNascimento}/>
            <TextoTitulos>E-mail:</TextoTitulos>
            <Input onChangeText={e=>setEmail(e)} value={email}/>

            
            <Botao onPress = {addList}>
              <TextButton>Cadastrar</TextButton>
            </Botao>

            <Botao onPress={listScreen}>
              <TextButton>Ver Cadastros</TextButton>
            </Botao>
        
        </BoxInputs>

      </Scroll>
    </Body>
  );
};
export default HomeScreen;
